...
1. If a `pyproject.toml` file containing the following is found:
    1. a `[build-system]` section containing `build-backend = "setuptools.build_meta"` and,
    1. `[project]` section with a `dynamic` key, where the corresponding list includes `"dependencies"` and,
    1. `[tool.setuptools.dynamic]` section containing `dependencies = { file = [some_requirements_file] }`,
