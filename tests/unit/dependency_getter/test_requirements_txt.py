...
def test_parse_requirements_file(tmp_path: Path) -> None:
    fake_requirements_file = """click==8.1.3 #123asd
colorama==0.4.5
...
"""
    with run_within_dir(tmp_path):
        with Path("requirements.txt").open("w") as f:
            f.write(fake_requirements_file)
...
def test_parse_requirements_file_urls(tmp_path: Path) -> None:
    fake_requirements_file = """urllib3 @ https://github.com/urllib3/urllib3/archive/refs/tags/1.26.8.zip
...
def test_single(tmp_path: Path) -> None:
    with run_within_dir(tmp_path):
        ...
        dependencies_extract = RequirementsTxtDependencyGetter(
            Path("pyproject.toml"), requirements_file=("req.txt",)
        ).get()
        dependencies = dependencies_extract.dependencies
...
def test_multiple(tmp_path: Path) -> None:
    with run_within_dir(tmp_path):
        ...
        dependencies_extract = RequirementsTxtDependencyGetter(
            Path("pyproject.toml"), requirements_file=("foo.txt", "bar.txt")
        ).get()
        dependencies = dependencies_extract.dependencies
...
def test_dev_multiple_with_arguments(tmp_path: Path) -> None:
    with run_within_dir(tmp_path):
        ...
        dependencies_extract = RequirementsTxtDependencyGetter(
            Path("pyproject.toml"), requirements_file_dev=("foo.txt", "bar.txt")
        ).get()
        dev_dependencies = dependencies_extract.dev_dependencies
