...
def test_requirements_file(tmp_path: Path) -> None:
    with run_within_dir(tmp_path):
...
def test_requirements_file_with_argument(tmp_path: Path) -> None:
    with run_within_dir(tmp_path):
...
def test_dependency_specification_not_found_raises_exception(tmp_path: Path) -> None:
    with run_within_dir(tmp_path), pytest.raises(
        ...
        DependencySpecificationDetector(Path("pyproject.toml"), requirements_file=("req/req.txt",)).detect()
