...
def test_cli_gitignore_not_used_when_using_exclude(pip_venv_factory: PipVenvFactory) -> None:
    with pip_venv_factory(Project.GITIGNORE) as virtual_env:
        ...
        assert get_issues_report(Path(issue_report)) == snapshot([
            ...
            {
                "error": {"code": "DEP001", "message": "'hello' imported but missing from the dependency definitions"},
                "module": "hello",
                "location": {"file": "src/barfoo.py", "line": 1, "column": 8},
            },
            {
                "error": {"code": "DEP001", "message": "'hej' imported but missing from the dependency definitions"},
                "module": "hej",
                "location": {"file": "src/baz.py", "line": 1, "column": 8},
            },
        ])
