...
def test_import_parser_py():
    imported_modules = ImportParser()._get_imported_modules_for_file(Path("tests/data/some_imports.py"))
    assert set(imported_modules) == set(["os", "pathlib", "typing", "pandas", "numpy"])


def test_import_parser_ipynb():
    imported_modules = ImportParser()._get_imported_modules_for_file(
