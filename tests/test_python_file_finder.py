...
def test_find_only_py_files():
    ...
    """
    files = PythonFileFinder(ignore_notebooks=True).get_all_python_files_in(Path("tests/data/example_project"))
    assert len(files) == 1
...
def test_find_py_and_ipynb_files():
    ...
    """
    files = PythonFileFinder(ignore_notebooks=False).get_all_python_files_in(Path("tests/data/example_project"))
    assert len(files) == 2
